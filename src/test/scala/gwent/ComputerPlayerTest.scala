...
class ComputerPlayerTest extends FunSuite {
  ...
  var player1: ComputerPlayer = null
  var player2: ComputerPlayer = null
  var player3: ComputerPlayer = null
  var player4: ComputerPlayer = null
  ...
  override def beforeEach(context: BeforeEach): Unit = {
    ...
    player1 = ComputerPlayer(name1, cardDeck1)
    player2 = ComputerPlayer(name2, 1, cardDeck2, cardHand, 3)
    player3 = ComputerPlayer(name1, 2, cardDeck1, cardHand, 2)
    player4 = ComputerPlayer(name1, 1, cardDeck1, Nil, 2)
  }
  test("Un ComputerPlayer debe tener name, section, cardDeck, cardHand(puede ser null) y gemstones. Ademas prueba los getters"){
    assertEquals(player1.getName, name1)
    assertEquals(player1.getSection, 2)
    ...
    assertEquals(player2.getSection, 1)
    assertEquals(player2.getCardDeck, cardDeck2)
    assertEquals(player2.getCardHand, cardHand)
    assertEquals(player2.getGemstones, 3)
  }

  test("Dos ComputerPlayer son iguales si tienen el mismo nombre y estan en la misma seccion (no pueden haber dos jugadores en la misma seccion ni con el mismo nombre)"){
